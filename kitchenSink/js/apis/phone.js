...
function displayCallDetail()
{
	try
	{
		...
		var calls = blackberry.phone.Phone.activeCalls();
		if (calls) 
		{
...
function displayPhoneIdentityDetails()
{
	try
	{
		...
		var lineIds = blackberry.phone.Phone.getLineIds();
...
function displayPhoneLogDetails()
{
	try
	{
		...
		var numCalls = blackberry.phone.PhoneLogs.numberOfCalls(blackberry.phone.PhoneLogs.FOLDER_NORMAL_CALLS);
		var numMissedCalls = blackberry.phone.PhoneLogs.numberOfCalls(blackberry.phone.PhoneLogs.FOLDER_MISSED_CALLS);
		...
		if (numMissedCalls > 0)
		{
			sb.append("<ul>");
			var callLog = null;
			for (var i = 0; i < numMissedCalls; i++)
			{
